...
REM Count total number of global_props.mp4 files in all subfolders, excluding _backup folders
for /R "%root_folder%" %%F in (global_props.mp4) do (
    echo %%~dpF | findstr /i "_backup\\" >nul
    if errorlevel 1 (
        set /a "total_files+=1"
    )
)
echo Total global_props.mp4 files found (excluding backup folders): %total_files%

REM Process each global_props.mp4 file in all subfolders, excluding _backup folders
for /R "%root_folder%" %%F in (global_props.mp4) do (
    echo %%~dpF | findstr /i "_backup\\" >nul
    if errorlevel 1 (
