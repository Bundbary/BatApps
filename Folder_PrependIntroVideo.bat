...
echo "this file is being taken over by a python script that also adds the audio."
pause
...
@REM set "filter_complex=[0:v]fps=!intro_framerate!,scale=1920:1080,fade=t=out:st=!fade_start!:d=1[v0];[2:v]fps=!intro_framerate!,fade=t=in:st=0:d=1[v1];[v0][1:a][v1][2:a]concat=n=2:v=1:a=1[outv][outa]"

@REM REM Execute optimized ffmpeg command
@REM ffmpeg -y ^
@REM -i "%intro_file%" ^
@REM -f lavfi -t !intro_duration! -i anullsrc=channel_layout=stereo:sample_rate=44100 ^
@REM -i "%main_file%" ^
